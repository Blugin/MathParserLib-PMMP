...
class CommandListener implements CommandExecutor{
    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args) : bool{
        ...
        if (!empty($expression)) {
            try{
                $sender->sendMessage($this->owner->getLanguage()->translate('commands.math.success', [Plugin::parse($expression)]));
            } catch (\Exception $exception){
                $sender->sendMessage($this->owner->getLanguage()->translate('commands.math.failure', [$exception->getMessage()]));
            }
            return true;
        }
        return false;
